<template>
  <div class="myinfo-container">
    <van-nav-bar title="分享" left-text="返回" fixed left-arrow @click-left="prepage" />
    <!-- 顶部留白 -->
    ...
    <button @click="webappPay">移动支付测试</button>
    <button @click="openurl">网页打开测试</button>
    <div>
      <div class="funcbtn">
  ...
  export default {
    ...
    methods: {
      ...
      webapp() {
        ...
        let signA = `appid=${this.appid}` + `&body=${this.body}` + `&mch_id=${this.mch_id}` +
          `&nonce_str=${this.nonce_str}` +
          `&notify_url=${this.notify_url}` + `&out_trade_no=${this.out_trade_no}` + `&scene_info=${scene_info}` +
          `&spbill_create_ip=${this.spbill_create_ip}` + `&total_fee=${this.total_fee}` +
